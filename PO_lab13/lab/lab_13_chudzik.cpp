...
public:
	...
	void wypis();
	Kamera& operator=(const Kamera& kk) {
		max_czas = kk.max_czas;
	...
	Kamera& operator+=(const Kamera& kk) {
		max_czas += kk.max_czas;
		strcat(standard, ", ");
		strcat(standard, kk.standard);
		return *this;
	}
	Kamera* nast;
protected:
	int max_czas;
	char standard[50]; //MPEG-2 MPEG-4 AVCHD HDV
};
